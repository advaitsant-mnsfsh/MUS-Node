...
interface ReportContainerProps {
  ...
  inputs?: AuditInput[];
  /** Split-view unlock: show report preview clearly; no fullscreen AuthBlocker (LoginPanel handles signup) */
  teaserMode?: boolean;
}
...
export const ReportContainer: React.FC<ReportContainerProps> = ({
  ...
  // --- DATA PREP (screenshots for PDFs) ---
  ...
  const {
    ...
    generateHybridPdf,
    isGenerating: isStandardPdfGenerating,
  ...
  const {
    ...
    competitorScreenshotUrl: competitorScreenshotSrc,
  });

  // Select the appropriate PDF generator based on report type
  const generatePdf = isCompetitorReport
    ? generateCompetitorPdf
    : generateStandardPdf;
  const isPdfGenerating = isCompetitorReport

...
export const CompetitorAnalysisView: React.FC<CompetitorAnalysisViewProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "Summary" | "UX" | "Product" | "Visual" | "Strategy" | "Accessibility"
  >("Summary");
  ...
  const ComparisonTable = ({
    ...
    if (!items || items.length === 0) {
      return (
        <div className="p-8 text-center text-slate-500 italic bg-slate-50 rounded-2xl border border-slate-200">
    ...
    return (
      <div
        className={`bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden ${!isPdf ? "animate-in fade-in slide-in-from-bottom-4" : "mb-8 break-inside-avoid pdf-item"}`}
      >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 text-slate-500 text-sm uppercase tracking-wider">
                ...
                <th className="p-4 font-semibold border-b border-slate-200 w-24 text-center">
                  You
                </th>
                <th className="p-4 font-semibold border-b border-slate-200 w-24 text-center">
                  Them
                ...
                <th className="p-4 font-semibold border-b border-slate-200 min-w-[300px]">
                  Analysis
                </th>
              </tr>
            </thead>
                ...
                ?.filter(
                  (item) =>
                    !(
                      ...
                      (item.Analysis.includes("N/A") ||
                        item.Analysis.includes("Not Applicable"))
                    ),
                )
                .map((item, idx) => (
                  <tr
                    key={idx}
                    className={
                      !isPdf ? "hover:bg-slate-50 transition-colors" : ""
                    }
                  >
                    ...
                    <td className="p-4 text-center font-bold text-indigo-600 bg-indigo-50/30">
                      {item.PrimaryScore}/10
                    </td>
                    <td className="p-4 text-center font-bold text-slate-600">
                      {item.CompetitorScore}/10
                    </td>
  ...
  return (
    <div className="space-y-6">
      ...
      {!isPdf && (
        <div className="bg-slate-100/50 p-1.5 rounded-xl flex overflow-x-auto space-x-1 border border-slate-200 no-scrollbar">
          {[
      ...
      {(activeTab === "Summary" || isPdf) && (
        <div
          className={`space-y-8 ${!isPdf ? "animate-in fade-in slide-in-from-bottom-4 duration-500" : ""}`}
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 break-inside-avoid pdf-item">
              ...
              <div className="space-y-4">
                {(!data.PrimaryStrengths ||
                  data.PrimaryStrengths.length === 0) && (
                ...
                {data.PrimaryStrengths?.map((str, i) => (
                  <div
                    key={i}
                    className="group p-4 rounded-xl border border-green-100 bg-green-50/30 hover:bg-green-50 hover:border-green-200 transition-all"
                  >
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 break-inside-avoid pdf-item">
              ...
              <div className="space-y-4">
                {(!data.CompetitorStrengths ||
                  ...
                  <p className="text-slate-400 italic text-sm">
                    No specific competitor strengths identified.
                  </p>
                )}
                {data.CompetitorStrengths?.map((str, i) => (
                  <div
                    key={i}
                    className="group p-4 rounded-xl border border-red-100 bg-red-50/30 hover:bg-red-50 hover:border-red-200 transition-all"
                  >
            ...
            <div className="p-8 relative z-10">
              <div className="flex items-center gap-4 mb-8 border-b border-indigo-400/30 pb-6">
                ...
                <div>
                  ...
                  <p className="text-indigo-100 mt-1">
                    Strategic opportunities to regain the lead.
                  </p>
                </div>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                ...
                {data.Opportunities?.map((opp, idx) => (
                  <div
                    key={idx}
                    className="bg-white/10 backdrop-blur-md border border-white/20 p-5 rounded-xl hover:bg-white/15 transition-all"
                  >
                    <h4 className="font-bold text-lg mb-3 text-white flex items-start gap-2">
                      <span className="opacity-60 font-mono text-sm mt-1">
                        0{idx + 1}
                      </span>
                      {opp.Opportunity}
                    </h4>
      ...
      {(activeTab === "Strategy" || isPdf) && (
        <ComparisonTable
          items={data.StrategyComparison}
          title="Strategic Positioning"
        />
      )}
      {(activeTab === "Accessibility" || isPdf) && (
        <ComparisonTable

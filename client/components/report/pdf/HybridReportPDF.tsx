...
const getScoreInfo = (score: number) => {
  ...
  if (score >= 5)
    return { color: COLORS.needsImprovement, label: "NEEDS IMPROVEMENT" };
...
const parseExecutiveSummary = (text: string = "") => {
  const workingMatch = text.match(
    /WHAT IS WORKING:([\s\S]*?)(?=WHAT IS (NOT WORKING|NEEDS WORK|NOT WORKING):|$)/i,
  );
  const needsWorkMatch = text.match(
    /WHAT IS (NOT WORKING|NEEDS WORK|NOT WORKING):([\s\S]*?)$/i,
  );
...
export const HybridReportPDF: React.FC<HybridReportPDFProps> = ({
  report,
  url,
  screenshots,
  whiteLabelLogo,
  ...
  return (
    <Document
      title={`UX Audit Report - ${url?.replace(/^https?:\/\//, "")?.replace(/\/$/, "") || "Analyzed Site"} (Hybrid)`}
    >
      <Page size="A4" style={styles.page}>
              ...
              <View style={styles.mainBadgeWrapper}>
                ...
                <View style={styles.mainBadgeMain}>
                  <Text style={styles.mainStatusText}>
                    {getScoreInfo(overallScore).label}
                  </Text>
                </View>
              </View>
            </View>
          ...
          <View style={styles.summaryRow}>
            ...
            <View style={styles.summaryCol}>
              ...
              {summary.working.length > 0 ? (
                summary.working.slice(0, 4).map((p, i) => (
                  <View key={i} style={styles.summaryPointContainer}>
                    <Text style={styles.summaryPoint}>
                      {p.split("(Citation:")[0].trim()}
                    </Text>
                    {p.includes("(Citation:") && (
                      <Text style={styles.citationSmall}>
                        Citation: {p.split("(Citation:")[1].replace(/\)$/, "")}
                      </Text>
                    )}
                  </View>
                ))
            ...
            <View style={styles.summaryCol}>
              ...
              {summary.needsWork.length > 0 ? (
                summary.needsWork.slice(0, 4).map((p, i) => (
                  <View key={i} style={styles.summaryPointContainer}>
                    <Text style={styles.summaryPoint}>
                      {p.split("(Citation:")[0].trim()}
                    </Text>
        ...
        <View style={styles.footer} fixed>
          <Text style={styles.footerTextLeft}>
            scores are AI generated and represent an overall assessment based on
            110+ factors
          </Text>
          <Text
            ...
            <View style={[styles.summaryCol, { width: "50%" }]}>
              <View style={styles.subHeaderRow}>
                ...
                {strategy?.PurposeAnalysis?.Confidence && (
                  <Text style={styles.contextConfidenceText}>
                    {strategy.PurposeAnalysis.Confidence} CONFIDENCE
                  </Text>
                )}
              </View>
              ...
              <View>
                ...
                <View style={{ marginTop: 4 }}>
                  <Text
                    style={{ fontSize: 9, lineHeight: 1.4, color: "#1e293b" }}
                  >
                    {strategy?.PurposeAnalysis?.KeyObjectives ||
                      "No key objectives identified."}
                  </Text>
                </View>
              </View>
            </View>

...
import { UserBadge } from './UserBadge'; // Reuse existing badge for now, can be styled further if needed
            ...
            <div className="w-full mx-auto px-4 sm:px-6 lg:px-8">
                <div className="relative flex justify-between items-center h-20">
                    ...
                    <div className="flex items-center gap-4 z-10">
                        {/* My Assessments - Moved to Right to preserve Center Stability */}
                        {user && (
                            <Link
                                to="/dashboard"
                                className={`hidden md:block text-sm font-semibold transition-colors mr-4 ${isActive('/dashboard')
                                    ? 'text-[#6366F1]'
                                    : 'text-[#64748B] hover:text-[#1E293B]'
                                    }`}
                            >
